...
A single-module Spring Boot 4.0.7 (Java 25) REST API for a "news" resource, backed by H2 with Liquibase-managed schema/seed data. It's a training/practice project (`spring-practice-2` per `pom.xml` artifactId), so expect a small, straightforward codebase without extensive layering.
