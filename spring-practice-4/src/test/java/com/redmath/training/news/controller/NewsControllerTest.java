...
class NewsControllerTest {
  ...
  void findAll_withoutPageParams_shouldReturnFirstPageWithDefaults() {
    try {
      mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/news")).andDo(print())
  ...
  void findAll_withPageParams_shouldReturnRequestedPageSortedAccordingly() {
    try {
      mockMvc.perform(
              MockMvcRequestBuilders.get("/api/v1/news").param("page", "1").param("size", "5")
                  .param("sortBy", "newsId").param("direction", "asc")).andDo(print())
  ...
  void shouldCreateNews() {
    try {
      mockMvc.perform(
              MockMvcRequestBuilders.post("/api/v1/news").contentType(MediaType.APPLICATION_JSON)
                  .content(
